...
namespace sdds {
    ...
    MenuItem::MenuItem(const char* itemContent) {
        if (itemContent != nullptr && itemContent[0] != '\0') {
            content = new char[ut.strlen(itemContent) + 1]; // Allocate memory for the content
            ut.strcpy(content, itemContent); // Copy the content to the allocated memory
        }
        else {
            setEmpty();
        }
    ...
    int Menu::run() {
        int userInput;
        displayMenu();
        userInput = ut.getInteger(0, itemCount); // Get user input within the range of 0 to item count
        return userInput;
    }

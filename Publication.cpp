...
namespace sdds {
	...
	Publication& Publication::operator=(const Publication& publication) {
		...
		if (publication.m_title) {
			m_title = new char[ut.strlen(publication.m_title) + 1];
			ut.strcpy(m_title, publication.m_title);
		}
		else {
			m_title = nullptr;
		}
	...
	bool Publication::operator==(const char* title) const {
		return ut.strstr(m_title, title) != nullptr;
	}
	...
	std::ostream& Publication::write(std::ostream& os) const {
		...
		if (conIO(os)) {
			...
			ut.strcpy(temp_title, m_title, SDDS_TITLE_WIDTH); // Copy the title to the temporary string
	...
	std::istream& Publication::read(std::istream& istr) {
		...
		if (conIO(istr)) {
			...
			if (ut.strlen(temp_shelfId) != SDDS_SHELF_ID_LEN)
		...
		if (!istr.fail()) {
			m_title = new char[ut.strlen(temp_title) + 1];
			ut.strcpy(m_title, temp_title);
